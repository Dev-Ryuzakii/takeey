import React, { useState } from 'react';
import { createClient } from '@supabase/supabase-js';
import { Link } from "react-router-dom"
import FormField from './UI/FormField';

// Initialize Supabase
const supabase = createClient(
  'https://pzsqhtacxltelnnssrhj.supabase.co', // Replace with your Supabase URL
  'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InB6c3FodGFjeGx0ZWxubnNzcmhqIiwicm9sZSI6ImFub24iLCJpYXQiOjE3MzM3MzU5MDUsImV4cCI6MjA0OTMxMTkwNX0.HoEXQ-ikrmxk_zoj9XQ70U1JC8RnoiN3GcGWLtl_8dY' // Replace with your Supabase anon key
);
const SignUp = () => {
  const handleGoogleSignIn = async () => {
    const { error } = await supabase.auth.signUp({
      provider: 'google',
    });
    if (error) console.error('Error signing in with Google:', error.message);
  };

  const handleMicrosoftSignIn = async () => {
    const { error } = await supabase.auth.signInWithOAuth({
      provider: 'microsoft',
    });
    if (error) console.error('Error signing in with Microsoft:', error.message);
  };

  const [showForm, setShowForm] = useState(false);
  const [email, setEmail] = useState("");
  const [isEmailValid, setIsEmailValid] = useState(false);

  // Email validation regex
  const validateEmail = (email) => {
    const regex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    return regex.test(email);
  };

 


  // Update email and validate it
  const handleEmailChange = (value) => {
    setEmail(value);
    setIsEmailValid(validateEmail(value));
  };

  const handleSignUp = async () => {
    if (isEmailValid) {
      const { user, error } = await supabase.auth.signUp({
        email: email,
      });

      if (error) {
        console.error("Error signing up:", error.message);
      } else {
        console.log("Check your email for the confirmation link.");
      }
    }
  };
  return (
    <>
      {/* Navigation Bar */}
      <nav className="flex items-center w-full h-24 bg-white drop-shadow-lg justify-center">
        <h1 className="text-3xl text-primary-500">Takeey</h1>
      </nav>

      {/* Main Content */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 my-36 mx-4 md:mx-28 container">
        {/* Text Section */}
        <div className="px-4 text-center">
          <h1 className="text-2xl font-bold">Create Account</h1>
          <p className="text-sm text-primary-400 font-semibold py-4">
            Making Note-Taking Easy and Organized for Everyone
          </p>
          <div className='mx-4 md:mx-44'>
            {/* Google Button */}
            <button
              onClick={handleGoogleSignIn}
              className="w-full md:w-96 rounded-lg flex items-center justify-center gap-4 h-12 border border-gray-300 hover:bg-gray-100 transition my-2"
            >
              <img
                src="data:image/png;base64,"
                alt="Google"
                className="w-6 h-6"
              />
              Continue with Google
            </button>

            {/* Microsoft Button */}
            <button
              onClick={handleMicrosoftSignIn}
              className="w-full md:w-96 rounded-lg flex items-center justify-center gap-4 h-12 border border-gray-300 hover:bg-gray-100 transition my-10"
            >
              <img
                src="https://upload.wikimedia.org/wikipedia/commons/4/44/Microsoft_logo.svg"
                alt="Microsoft"
                className="w-6 h-6"
              />
              Continue with Microsoft
            </button>
            {/* Divider */}
            <div className="flex items-center my-6 mx-10">
              <hr className="w-28 border-gray-300" />
              <span className="mx-4 text-gray-500">OR</span>
              <hr className="w-28 border-gray-300" />
            </div>
          </div>
          {/* form Section */}
          <div>
      {/* Initially show text, then toggle to show form */}
      {showForm ? (
        <div
          className="space-y-6 mx-36 transition-opacity duration-500 ease-in-out max-w-sm w-full"
          style={{ opacity: showForm ? 1 : 0 }}
        >
          <form>
            <FormField
              title=""
              value={email}
              placeholder="Email"
              handleChange={(event) => {
                handleEmailChange(event);  // Update email value
                handleSignUp(event);       // Perform signup logic
              }}
            />
          </form>

          {/* Continue Button */}
          {showForm && isEmailValid && (
           <Link to="/password">
           <button
             type="button"
             className="mt-4 w-full text-white bg-primary-500 py-2 rounded-lg text-lg font-semibold disabled:bg-gray-400"
             disabled={!isEmailValid}
           >
             Continue
           </button>
         </Link>
          )}
        </div>
      ) : (
        <div
          className="text-center transition-opacity duration-500 ease-in-out"
          style={{ opacity: showForm ? 0 : 1 }}
        >
          <button
            type="button"
            onClick={() => setShowForm(true)}
            className="text-primary-500 text-sm sm:text-base md:text-xl font-bold px-4 py-2 sm:px-6 sm:py-3 whitespace-nowrap"
          >
            Other way to log In
          </button>
        </div>
      )}
    </div>
          <p className="my-3 text-gray-500 text-sm sm:text-base md:text-lg">
          Already have an Otter Account?{" "}
  <Link to="/" className="font-semibold text-primary-500">
    Sign In
  </Link>
</p>
          <p className="text-gray-500 my-8 text-sm sm:text-base md:text-lg px-4 text-center sm:px-8">
            By using Takeey you agree to the{" "}
            <Link to="" className="text-primary-500 hover:underline">
              Terms of Service{" "}
            </Link>
            and{" "}
            <Link to="" className="text-primary-500 hover:underline">
              Privacy Policy
            </Link>
          </p>

        </div>

        {/* Image Section */}
        <div className="p-4 flex justify-center items-center hidden md:block">
          <div className="h-[500px] w-full">
            <img
              src="/src/assets/Taking notes-cuate.png"
              alt="A person taking notes"
              className="w-full h-full object-cover rounded-lg"
            />
          </div>
        </div>
      </div>
    </>
  );
};

export default SignUp;
